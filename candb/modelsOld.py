...
class Profile(AbstractUser):
    ...
    """
    phone = models.CharField(max_length=22, help_text="Phone Number", null=True,
                             blank=True)  # Uses format +CCCCC/NNNNNNNNNNNNNNN where CC is country code and NNNNNNNNNNNNNNN is the phone number
    image = models.ImageField(default='default.jpg', upload_to='profile_pics', blank=True, null=True)
    balance = models.DecimalField(max_digits=cfg.Profile.MAXIMUM_BALANCE_DIGITS,
                                  decimal_places=cfg.Profile.BALANCE_DECIMAL_DIGITS, help_text="User Balance",
                                  null=False, blank=False, default=0)
    adminNotes = models.TextField(help_text="Admin Notes", null=True, blank=False, default=None,
                                  max_length=cfg.Profile.MAXIMUM_ADMIN_NOTES_LENGTH)
    _saveVersion = IntegerVersionField(help_text="Save Version for Concurrency Control")

    @classmethod
    def create(cls: Union[Self, Callable], username: str, first_name: str = None, last_name: str = None, *_,
               email: str = None,
        ...
        if phoneNCountryCode is not None and (phoneNCountryCode.__len__() > 5 or 3 > pNumber.__len__() > 15):
            raise ValueError(
                "phoneNCountryCode must no more than 5 characters long, and pNumber must be between 3 and 15 characters long")
        ...
        if is_superuser:
            match input(
                "WARNING: You are creating a superuser account. This is VERY dangerous. Are you sure? (y/[N] - case-sensitive) > "):
                case 'y':
                ...
                case _:
                    raise ValueError(
                        "superuser creation cancelled as per user request. Again: this is not recommended.")
    ...
    def subtractBalance(self: Self, amount: float | Decimal, autosave: bool = cfg.DEFAULT_AUTOSAVE_MODE,
        ...
        """
        if not self._checkBalance(amount, overrideAllowNegativeBalance=overrideAllowNegativeBalance,
                                  overrideMaximumNegativeBalance=overrideMaximumNegativeBalance):
            if raiseException:
...
class Order(models.Model):
    ...
    """
    id = models.CharField(primary_key=True, editable=False, help_text="Unique Order ID, same across databases",
                          null=False, blank=False, unique=True, max_length=42)  # ORDER-UUID4 (len 42)
    orderTime = models.DateTimeField(help_text="Order Time", null=False, blank=False)
    overrideCost = models.DecimalField(max_digits=cfg.Order.MAXIMUM_COST_DIGITS,
                                       decimal_places=cfg.Order.COST_DECIMAL_DIGITS, help_text="Override Cost",
                                       null=True, blank=False, default=None)  # If None, means not overridden.
    totalCost = models.DecimalField(max_digits=cfg.Order.MAXIMUM_COST_DIGITS,
                                    decimal_places=cfg.Order.COST_DECIMAL_DIGITS,
                                    help_text="Order Total Practical Price",
                                    null=True)  # If None, means not calculated yet.
    totalPaid = models.DecimalField(max_digits=cfg.Order.MAXIMUM_COST_DIGITS,
                                    decimal_places=cfg.Order.COST_DECIMAL_DIGITS, help_text="Total Paid", null=False,
                                    blank=False, default=0)  # Total amount paid by the user
    notes = models.TextField(help_text="Order Notes", null=True, blank=False, default=None,
                             max_length=cfg.Order.MAXIMUM_NOTES_LENGTH)
    ...
    class Meta:
        ...
        permissions = [("view_any_order", "Can view any order regardless of its owner"),
                       ("change_any_order", "Can change any orders regardless of its owner"),
                       ("delete_any_order", "Can delete orders regardless of its owner"),
                       ("add_any_order", "Can add any order regardless of its owner"), (
                       "change_any_order_but_overridecost",
                       "Can change orders regardless of its owner, but cannot override the cost")]
        ...
        constraints = [
            ...
            models.CheckConstraint(check=models.Q(totalCost__gte=0), name="CanDB_Order_TotalCost_NonNegative",
                                   violation_error_code="ORDER-TOTALCOST-1",
            ...
            models.CheckConstraint(check=models.Q(overrideCost__gte=0), name="CanDB_Order_OverrideCost_NonNegative",
                                   ...
                                   violation_error_message="Total cost of an order cannot negative"),
            models.CheckConstraint(check=models.Q(id__startswith="ORDER-"), name="CanDB_Order_ID_Prefix",
                                   violation_error_code="ORDER-ID-1",
                                   violation_error_message="Order ID must start with 'ORDER-'"),
            models.CheckConstraint(check=models.Q(id__len=42), name="CanDB_Order_ID_Len",
                                   violation_error_code="ORDER-ID-2",
                                   violation_error_message="Order ID must be 42 characters long"),
        ]
    ...
    def calculateTotalCost(self: Self, raiseException: bool = False,
                           autosave: bool = cfg.DEFAULT_AUTOSAVE_MODE) -> float | NoReturn | bool:
        """
...
class Product(models.Model):
    ...
    """
    id = models.CharField(max_length=44, primary_key=True, help_text="Unique Product ID, same across databases",
                          null=False, blank=False, unique=True)  # PRODUCT-UUID4 (len 44)
    name = models.CharField(max_length=cfg.Product.MAXIMUM_NAME_LENGTH, help_text="Product Name", null=False,
                            blank=False)
    price = models.DecimalField(max_digits=cfg.Product.MAXIMUM_COST_DIGITS,
                                decimal_places=cfg.Product.COST_DECIMAL_DIGITS, help_text="Product Price", null=False,
                                blank=False)
    description = models.TextField(max_length=cfg.Product.MAXIMUM_DESCRIPTION_LENGTH, help_text="Product Description",
    ...
    availability = models.JSONField(help_text="Product Availability Configuration", null=True, blank=False,
                                    default=None, encoder=_common.DateTimeEncoder,
                                    decoder=_common.DateTimeDecoder)  # Use None for no availability: always use model stock.
    ...
    """
    notes = models.TextField(help_text="Product Notes", null=True, blank=False, default=None,
                             max_length=cfg.Product.MAXIMUM_NOTES_LENGTH)
    tags = models.JSONField(max_length=cfg.Product.MAXIMUM_PRODUCT_TAGS, help_text="Product Tags", null=False,
                            blank=True)
    _saveVersion = IntegerVersionField(help_text="Save Version for Concurrency Control")
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(check=models.Q(physicalStock__isnull=False) | (
                        models.Q(physicalStock__isnull=True) & ~ models.Q(reservedStock__isnull=True)),
    ...
    @classmethod
    def create(cls: Union[Self, Callable], name: str, price: float, description: str, image: str = None,
               physicalStock: int | None = 0,
        ...
        if physicalStock is None:
            if reservedStock is not None:
                raise ValueError(
                    "reserved stock must be None when physical stock is None as None indicates infinite stock")
            ...
            raise TypeError("bad type for reservedStock%s" % (
                " (hint: reservedStock must be int if physicalStock is int)" if physicalStock is not None else ''))
    ...
    @deprecated(details="Stock is now managed by TimeBracket and StockConfig")
    def _stockAvailableForStockConfig(self,
                                      _config: _common.AvailabilityConfiguration) -> _common.AvailabilityIndicator:
        """
    ...
    def checkProductStock(self, quantityRequired: int, attemptUntilStockFound: bool = False, findMaximum: bool = False,
        ...
        if findMaximum and not attemptUntilStockFound:
            warnings.warn(
                "findMaximum is True but attemptUntilStockFound is False. The function's programming means that it will imply attemptUntilStockFound is True, even though it is not.",
                Warning)
    ...
    @deprecated(details="Stock is now managed by TimeBracket and StockConfig")
    def modifyReservedStock(self: Self, usingRefID: int | None, by: int, raiseException: bool = False,
                            autostartTransaction: bool = True,
                            autosave: bool = cfg.DEFAULT_AUTOSAVE_MODE) -> bool | NoReturn:
        """
...
class Vendor(models.Model):
    ...
    Product vendors.
    """
    id = models.CharField(max_length=43, primary_key=True, help_text="Unique Vendor ID", null=False, blank=False,
                          unique=True)  # VENDOR-UUID (len 43)
    name = models.CharField(max_length=cfg.Vendor.MAXIMUM_NAME_LENGTH, help_text="Vendor Name", null=False, blank=False)
    vendorAdmin = models.ForeignKey(help_text="Administrator account for vendor", to=Profile, blank=False, null=False,
...
class StockConfig(models.Model):
    ...
    id = models.CharField(max_length=42, primary_key=True,
                          help_text="Unique Stock Configuration ID, same across databases", null=False, blank=False,
                          unique=True)  # STOCK-UUID4 (len 42)
    ...
    reference = models.ForeignKey(help_text="Reference to another Stock Configuration", blank=True, null=True,
                                  on_delete=models.CASCADE, to="self")  # Not required
    ...
    def _modifyPhysicalStock(self: Self, by: int = None, autosave: bool = True) -> int:
        ...
        if by < 0 and (self.physicalStock + by) < 0 or (self.physicalStock + by) < self.remainingStock:
            raise _common.ExceedsModelLimits(
                "physicalStock must be larger or equal to remainingStock and cannot be smaller than 0")
...
class TimeBracket(models.Model):
    ...
    @classmethod
    def create(cls: Union[Self, Callable], linkedProduct: Product, linkedStockConfig: StockConfig,
        ...
        if priority is not None:
            ...
            if enableWarnings and cls.objects.filter(linkedProduct=linkedProduct, priority=priority).exists():
                warnings.warn(
                    "Another TimeBracket with the same linkedProduct and the same priority level already exists. "
        ...
        match bracketType:
            ...
            case cls.BracketType.DEFAULT:
                if cls.objects.filter(linkedProduct=linkedProduct, bracketType=cls.BracketType.DEFAULT).exists():
                    raise _common.MultipleDefaultBrackets(
                        "brackets linked to given product can only have one default bracket",
                        hint="is there a bracket with linkedProduct that is already using DEFAULT bracketType?")
                if enableWarnings and priority == 0:
                    warnings.warn(
                        "TimeBracket with bracketType DEFAULT should never be first priority bracket (it should be a fallback). "
...
class OrderLine(models.Model):
    ...
    """
    id = models.CharField(primary_key=True, help_text="Unique OrderLine ID, same across databases", null=False,
                          blank=False, unique=True, max_length=46)  # ORDERLINE-UUID4 (len 46)
    linkedOrder = models.ForeignKey(Order, on_delete=models.CASCADE, help_text="Order ID", default=-1, null=False,
                                    blank=False)
    linkedProduct = models.ForeignKey(Product, on_delete=models.CASCADE, help_text="Product ID", default=-1, null=False,
                                      blank=False)
    ...
    persistentCost = models.DecimalField(max_digits=cfg.OrderLine.MAXIMUM_COST_DIGITS,
                                         decimal_places=cfg.OrderLine.COST_DECIMAL_DIGITS, help_text="Persistent Cost",
                                         null=True,
                                         blank=False)  # When None, use linkedProduct.price. Means transaction has not been calculated yet; the customer is still shopping. Price of EACH product object.
    itemCost = models.DecimalField(max_digits=cfg.OrderLine.MAXIMUM_COST_DIGITS,
                                   decimal_places=cfg.OrderLine.COST_DECIMAL_DIGITS, help_text="Total Cost", null=True,
                                   blank=False,
                                   default=0)  # Cost of entire orderline (i.e., quantity * persistentCost, but overridden by forcePrice). When None, indicates a persistent cost has not been calculated yet. Separate from forcePrice (often still calculated, even if forcePrice set)
    # When using forcePrice, the persistentCost should still be set in case force price is retracted
    forcePrice = models.DecimalField(max_digits=cfg.OrderLine.MAXIMUM_COST_DIGITS,
                                     decimal_places=cfg.OrderLine.COST_DECIMAL_DIGITS,
                                     help_text="Force Price of Entire OrderLine", null=True, blank=False,
                                     default=None)  # Same as persistent cost, however, set when the price is overridden by an admin. Ignores quantity (i.e., the total cost of the orderline, not of each product object). If None, use persistentCost.
    costPaid = models.DecimalField(max_digits=cfg.OrderLine.MAXIMUM_COST_DIGITS,
                                   decimal_places=cfg.OrderLine.COST_DECIMAL_DIGITS, help_text="Cost Paid", null=False,
                                   blank=False, default=0)  # Total amount paid by the user for this orderline
    status = models.CharField(max_length=cfg.OrderLine.MAXIMUM_LENGTH_OF_ORDERLINE_STATUS_CHOICES,
                              choices=ORDERLINE_STATUS_AS_DICT, default=cfg.OrderLine.DEFAULT_STATUS,
                              help_text="Order Status", null=True, blank=False)
    notes = models.TextField(max_length=cfg.OrderLine.MAXIMUM_NOTES_LENGTH, help_text="Order Line Notes", null=True,
                             blank=False, default=None)
    stockContributions = models.JSONField(
        help_text="ID of Stock Configurations used in linked Product for availability", null=True, blank=False,
        default=None)
    """
    Uses format:
    ```python
    ...
    class Meta:
        ...
        permissions = [
            ...
            ("change_any_orderline_but_forceprice",
             "Can change any orderlines regardless of its owner, but cannot force a price"),
    ...
    @classmethod
    def create(cls: Union[Self, Callable], linkedOrder: Order, linkedProduct: Product, quantity: int,
               persistentCost: float = None,
               itemCost: float = None, forcePrice: float = None, status: str = cfg.OrderLine.DEFAULT_STATUS,
               notes: str = None, stockContributions: dict | None = None,
               autosave: bool = cfg.DEFAULT_AUTOSAVE_MODE) -> Self:
        """
    ...
    def _checkNotLocked(self: Self, raiseException: bool = True) -> NoReturn | bool:
        ...
        if self.status in _common.OrderLineStatus.Locked:
            if raiseException:
                raise _common.Locked(f"orderline is locked",
                                     hint="check if the orderline is locked before performing operations",
                                     modelObj=self)
            return False
        return True
    ...
    def _cancel(self: Self, raiseExcPasson: bool, autosavePasson: bool) -> NoReturn | None | bool:
        ...
        if self.availabilityID == -1:
            if self.quantityReserved != 0:  # If availabilityID not set but there is quantity reserved (should NEVER happen)
                raise _common.AvailabilityReferenceNotSet("availability reference not set", modelObj=self,
                                                          hint="is self.availabilityID set? Has the stock been reserved properly?")
        elif not self.linkedProduct.modifyReservedStock(self.availabilityID, -self.quantityReserved,
                                                        raiseException=raiseExcPasson,
    ...
    def cancel(self: Self, autostartTransaction: bool = True,
               autosave: bool = cfg.DEFAULT_AUTOSAVE_MODE) -> NoReturn | bool:
        """
    ...
    def _reserveStock(self: Self, forceAll: bool = False, atLeast: int = None, atMost: int = None,
        ...
        if not available:
            raise _common.InsufficientStock(
                f"insufficient stock to satisfy minimum quantity requirements of {self.__repr__()} (with minimum quantity requirement of {minQty})")
        ...
        for refID, qty in contributions.items():
            if not self.linkedProduct.modifyReservedStock(
                    ...
                    # Remember, a transaction should have already been started by the caller.
                ...
                raise RuntimeError(
                    f"critical error in stock reservation; False returned but no exception raised (while reserving stock for {self.__repr__()})? This should NEVER occur")
    ...
    def reserveStockFromProduct(self: Self, *_, forceAll: bool = False, atLeast: int = None, atMost: int = None,
                                exact: int = None, overrideAttemptUntilStockFound: bool = None,
                                overrideFindMaximumStockAvailable: bool = None,
                                autosave: bool = cfg.DEFAULT_AUTOSAVE_MODE,
                                autostartTransaction: bool = True) -> int | NoReturn:
        """
        ...
        if atMost is not None:
            ...
            if overrideFindMaximumStockAvailable is False:  # Please use 'overrideFindMaximumStockAvailable == False' instead of 'not overrideAttemptUntilStockFound' as while the first only checks for False, the second checks for False and None.
                warnings.warn(
                    "atMost is set, but overrideFindMaximumStockAvailable is False. This will break the functionality. It is highly recommended to set overrideFindMaximumStockAvailable to True, or None (default) so all stock configurations are queried.",
                    Warning)
        if overrideAttemptUntilStockFound is False and (
                atLeast is not None or atMost is not None):  # Please keep '== False'. Same comment as above.
            warnings.warn(
                "atMost and/or atLeast is set, but overrideAttemptUntilStockFound is False. This will break the functionality. It is highly recommended to set overrideAttemptUntilStockFound to True, or None (default) so all stock configurations are queried.",
                Warning)
...
class Transaction(models.Model):
    ...
    linkedOrderLine = models.ForeignKey(OrderLine, on_delete=models.CASCADE, help_text="OrderLine ID", default=-1,
                                        null=False, blank=False)
    linkedStockConfig = models.ForeignKey(StockConfig, on_delete=models.PROTECT, help_text="Stock Configuration ID",
                                          default=-1, null=False, blank=False)
    quantityReserved = models.PositiveIntegerField(help_text="Quantity Reserved from linkedStockConfig", null=False,
...
def __devEnvCreator() -> bool:
    ...
    try:
        Profile.create(forceID=1, username="anony", first_name="Anony", last_name="Mous", email="anony@mous.com",
                       password="1234", is_staff=False, is_superuser=False, image=None, requireSecurePassword=False)
    ...
    try:
        stf = Profile.create(forceID=2, username="staff", first_name="Staff", last_name="Member",
                             email="staff@candb.com", is_staff=True, is_superuser=False, image=None)
    ...
    try:
        ...
        prod = Product.create(name="Test Product", price=1.00, description="Test Product", image=None, physicalStock=10,
                              reservedStock=0, notes="Test Product", tags=["test"], autosave=True,
                              availability={
                                  1: ((datetime.now(tz=TZ_INFO), datetime.now(tz=TZ_INFO) + timedelta(days=1)),
                                      {"available": True, "physicalStock": 10, "reservedStock": 0, "infinite": False,
                                       "useModelStock": False}),
                              })
        prod = Product.create(name="Advanced Product 1", price=6.45, description="Test Product", image=None,
                              physicalStock=13,
                              reservedStock=0, notes="Test Product", tags=["test"], autosave=True,
                              availability={
                                  1: (
        ...
        class Profile(AbstractUser):
            ...
            def create(cls: Union[Self, Callable], username: str, first_name: str = None, last_name: str = None, *_,
                       email: str = None,
                       phoneNCountryCode: str = None, pNumber: str = None, password: str = None, is_staff: bool = False,
                       is_superuser: bool = False, image: str = None, adminNotes: str = None,
                       overrideBalance: float = None,
                       forceID: int = None, requireSecurePassword: bool = True,
                       autosave: bool = cfg.DEFAULT_AUTOSAVE_MODE) -> Self:
                """
                Create a new user profile
                        ...
                        case 'y':
                            ...
                            if not Profile.objects.filter(username=username).exists():
                                raise PermissionError(
                                    "superuser creation cancelled as no user exists with that username")
                            supposedSuperuser = Profile.objects.get(username=username)
                            if not supposedSuperuser.is_superuser:
                                raise PermissionError("superuser creation cancelled as user is not superuser")
                            if not supposedSuperuser.check_password(password):
            ...
            def subtractBalance(self: Self, amount: float | Decimal, autosave: bool = cfg.DEFAULT_AUTOSAVE_MODE,
                ...
                if not self._checkBalance(amount, overrideAllowNegativeBalance=overrideAllowNegativeBalance,
                                          overrideMaximumNegativeBalance=overrideMaximumNegativeBalance):
                    if raiseException:
                        raise _common.InsufficientFunds(
                            f"insufficient funds to subtract {amount} from {self.__repr__()}")
                    return False
                if amount < 0:
                    raise ValueError("amount must be non-negative")
                self.balance -= amount
        ...
        class Order(models.Model):
            ...
            totalCost = models.DecimalField(max_digits=cfg.Order.MAXIMUM_COST_DIGITS,
                                            ...
                                            null=True)  # If None, means not calculated yet.
            totalPaid = models.DecimalField(max_digits=cfg.Order.MAXIMUM_COST_DIGITS,
                                            decimal_places=cfg.Order.COST_DECIMAL_DIGITS, help_text="Total Paid",
                                            null=False, blank=False, default=0)  # Total amount paid by the user
            notes = models.TextField(help_text="Order Notes", null=True, blank=False, default=None,
                                     max_length=cfg.Order.MAXIMUM_NOTES_LENGTH)
            user = models.ForeignKey(help_text="User ID", blank=False, null=False, on_delete=models.CASCADE, to=Profile)
            ...
            class Meta:
                ...
                constraints = [
                    ...
                    models.CheckConstraint(check=models.Q(totalCost__gte=0), name="CanDB_Order_TotalCost_NonNegative",
                                           violation_error_code="ORDER-TOTALCOST-1",
                                           violation_error_message="Total cost of an order cannot negative"),
                    models.CheckConstraint(check=models.Q(overrideCost__gte=0),
                                           name="CanDB_Order_OverrideCost_NonNegative",
                                           violation_error_code="ORDER-OVERRIDE-1",
                                           violation_error_message="Total cost of an order cannot negative"),
                    models.CheckConstraint(check=models.Q(id__startswith="ORDER-"), name="CanDB_Order_ID_Prefix",
                                           violation_error_code="ORDER-ID-1",
        ...
        class Product(models.Model):
            ...
            name = models.CharField(max_length=cfg.Product.MAXIMUM_NAME_LENGTH, help_text="Product Name", null=False,
                                    blank=False)
            price = models.DecimalField(max_digits=cfg.Product.MAXIMUM_COST_DIGITS,
                                        decimal_places=cfg.Product.COST_DECIMAL_DIGITS, help_text="Product Price",
                                        null=False, blank=False)
            description = models.TextField(max_length=cfg.Product.MAXIMUM_DESCRIPTION_LENGTH,
                                           help_text="Product Description", null=True, blank=False)
            image = models.ImageField(upload_to="product_images", help_text="Product Image", null=True)
            # Available stock is equal to physicalStock - reservedStock.
            availability = models.JSONField(help_text="Product Availability Configuration", null=True, blank=False,
                                            default=None, encoder=_common.DateTimeEncoder,
            ...
            @deprecated(details="Stock is now managed by TimeBracket and StockConfig")
            @staticmethod
            def canContributeQty(offer: _common.AvailabilityConfiguration,
                                 maximum: _common.AvailabilityConfiguration) -> int:
                """
                Check how much an '`offer`' can contribute towards a total (i.e., `maximum`).
            ...
            @deprecated(details="Stock is now managed by TimeBracket and StockConfig")
            def checkProductStock(self, quantityRequired: int, attemptUntilStockFound: bool = False,
                                  findMaximum: bool = False,
                                  prioritiseInfinite: bool = False, overrideTime: datetime = None
                                  ) -> tuple[bool, int | bool | None, dict[int: int] | None]:
                """
                Check if the product has enough stock to sell.
        ...
        class Vendor(models.Model):
            ...
            Product vendors.
            """
            id = models.CharField(max_length=43, primary_key=True, help_text="Unique Vendor ID", null=False,
                                  blank=False, unique=True)  # VENDOR-UUID (len 43)
            name = models.CharField(max_length=cfg.Vendor.MAXIMUM_NAME_LENGTH, help_text="Vendor Name", null=False,
                                    blank=False)
            vendorAdmin = models.ForeignKey(help_text="Administrator account for vendor", to=Profile, blank=False,
        ...
        class StockConfig(models.Model):
            ...
            """
            id = models.CharField(max_length=42, primary_key=True,
                                  help_text="Unique Stock Configuration ID, same across databases", null=False,
                                  blank=False, unique=True)  # STOCK-UUID4 (len 42)
            vendor = models.ForeignKey(help_text="Vendor ID", blank=True, null=True, on_delete=models.CASCADE,
                                       to=Vendor)
            available = models.BooleanField(help_text="Product Availability", null=False, blank=False, default=True)
            reference = models.ForeignKey(help_text="Reference to another Stock Configuration", blank=True, null=True,
                                          on_delete=models.CASCADE, to="self")  # Not required
            infinite = models.BooleanField(help_text="Infinite Stock", null=True, blank=True)  # Not required
            physicalStock = models.PositiveIntegerField(help_text="Physical Stock", null=True,
                                                        blank=True)  # Not required
            remainingStock = models.PositiveIntegerField(help_text="Remaining Stock", null=True,
                                                         blank=True)  # Not required
            _saveVersion = IntegerVersionField(help_text="Save Version for Concurrency Control")
            ...
            @classmethod
            def create(cls: Union[Self, Callable], *_, vendor: Vendor = None, available: bool = True,
                       reference: Self = None, useModelStock: bool = None, infinite: bool = None,
                       physicalStock: int = None,
                       remainingStock: int = None, forceID: str = None,
                       autosave: bool = cfg.DEFAULT_AUTOSAVE_MODE) -> Self:
                if _:
                    raise TypeError("additional positional arguments provided when not accepted")
                # Available
                ...
                if not atLeastOneDetailPresent:
                    raise _common.NoStockTypeSpecified(
                        "expected at least one stock specified in stock configuration creation",
                        hint="specify at least one setting (i.e., reference, useModelStock, infinite, physicalStock/reserveStock)")
            ...
            def _modifyRemainingStock(self: Self, by: int = None, autosave: bool = True) -> int:
                ...
                """
                if not isinstance(self.physicalStock, int) or not isinstance(self.remainingStock, int):
                    raise _common.IncorrectStockTypeSpecified(
                        "expected physicalStock and remainingStock to be type int",
                        hint="is this StockConfig using the correct config type?")
                if not isinstance(by, int):
                    raise TypeError("expected type int for by")
                if by > 0:
                    if self.remainingStock + by > self.physicalStock:
                        raise _common.ExceedsModelLimits(
                            "remainingStock cannot exceed physicalStock under any circumstance",
                            hint=f"did you mean to deduct {by} from remainingStock? (If so, use {-by}.)")
                else:
                    if self.remainingStock + by < 0:
                        raise _common.InsufficientStock("insufficient stock to fulfill modification")
        ...
        class TimeBracket(models.Model):
            ...
            def create(cls: Union[Self, Callable], linkedProduct: Product, linkedStockConfig: StockConfig,
                ...
                if priority is not None:
                    ...
                    if not 0 <= priority <= 32767:
                        raise _common.ExceedsModelLimits(
                            "priority exceeds the value limits for PositiveSmallIntegerField",
                            hint="priority needs to be within range of 0 to 32767")
                    if enableWarnings and cls.objects.filter(linkedProduct=linkedProduct, priority=priority).exists():
                        warnings.warn(
                            "Another TimeBracket with the same linkedProduct and the same priority level already exists. "
                ...
                match bracketType:
                    case cls.BracketType.DATETIME:
                        if startDatetime is None or endDatetime is None:
                            raise TypeError(
                                "startDatetime and endDatetime must be specified when bracketType is DATETIME")
                        if not isinstance(startDatetime, datetime) or not isinstance(endDatetime, datetime):
                            raise TypeError("expected type datetime for startDatetime and endDatetime")
                        if startDatetime > endDatetime:
                            raise ValueError("startDatetime must be before or equal to endDatetime, not after")
                    ...
                    case cls.BracketType.WEEKDAY:
                        ...
                        additionalKwargs["applicableWeekdays"] = applicableWeekdays
                    case cls.BracketType.DEFAULT:
                        if cls.objects.filter(linkedProduct=linkedProduct,
                                              bracketType=cls.BracketType.DEFAULT).exists():
                            raise _common.MultipleDefaultBrackets(
                                "brackets linked to given product can only have one default bracket",
                                hint="is there a bracket with linkedProduct that is already using DEFAULT bracketType?")
                        if enableWarnings and priority == 0:
            ...
            @classmethod
            def applicableBracketsForProduct(cls: Self, product: Product, overrideTimestamp: datetime = None) -> tuple[
                Self]:
                """
                Returns time bracket with foreign-key reference to `product`, where `linkedStockConfig.available == True` and
                 within datetime range or is default bracket. Ordered by priority, ascending.
                ...
                if overrideTimestamp:
                    if not isinstance(overrideTimestamp, datetime):
                        raise TypeError(
                            f"expected overrideTimestamp to be type datetime, not {overrideTimestamp.__class__}")
                    tmstmp: datetime = overrideTimestamp
                else:
                    tmstmp: datetime = datetime.now(tz=TZ_INFO)
                brackets = cls.objects.filter(linkedProduct=product, linkedStockConfig__available=True).order_by(
                    "priority")
                applicable: list[Self] = list()
                for brkt in brackets:
                    if brkt._checkIsApplicableAtTime(tmstmp):
            ...
            def modifyWeekday(self: Self, add: set[int] = None, subtract: set[int] = None, autosave: bool = True) -> \
            set[int]:
                """
                Modify applicable weekdays for bracket.
        ...
        class OrderLine(models.Model):
            ...
            id = models.CharField(primary_key=True, help_text="Unique OrderLine ID, same across databases", null=False,
                                  blank=False, unique=True, max_length=46)  # ORDERLINE-UUID4 (len 46)
            linkedOrder = models.ForeignKey(Order, on_delete=models.CASCADE, help_text="Order ID", default=-1,
                                            null=False, blank=False)
            linkedProduct = models.ForeignKey(Product, on_delete=models.CASCADE, help_text="Product ID", default=-1,
                                              null=False, blank=False)
            quantity = models.PositiveIntegerField(help_text="Quantity", null=False, blank=False, default=1)
            quantityReserved = models.PositiveIntegerField(help_text="Quantity Reserved", null=False, blank=False,
                                                           default=0)
            persistentCost = models.DecimalField(max_digits=cfg.OrderLine.MAXIMUM_COST_DIGITS,
                                                 decimal_places=cfg.OrderLine.COST_DECIMAL_DIGITS,
                                                 help_text="Persistent Cost", null=True,
                                                 blank=False)  # When None, use linkedProduct.price. Means transaction has not been calculated yet; the customer is still shopping. Price of EACH product object.
            itemCost = models.DecimalField(max_digits=cfg.OrderLine.MAXIMUM_COST_DIGITS,
                                           decimal_places=cfg.OrderLine.COST_DECIMAL_DIGITS, help_text="Total Cost",
                                           null=True, blank=False,
                                           default=0)  # Cost of entire orderline (i.e., quantity * persistentCost, but overridden by forcePrice). When None, indicates a persistent cost has not been calculated yet. Separate from forcePrice (often still calculated, even if forcePrice set)
            # When using forcePrice, the persistentCost should still be set in case force price is retracted
            forcePrice = models.DecimalField(max_digits=cfg.OrderLine.MAXIMUM_COST_DIGITS,
                                             ...
                                             default=None)  # Same as persistent cost, however, set when the price is overridden by an admin. Ignores quantity (i.e., the total cost of the orderline, not of each product object). If None, use persistentCost.
            costPaid = models.DecimalField(max_digits=cfg.OrderLine.MAXIMUM_COST_DIGITS,
                                           decimal_places=cfg.OrderLine.COST_DECIMAL_DIGITS, help_text="Cost Paid",
                                           null=False, blank=False,
                                           default=0)  # Total amount paid by the user for this orderline
            status = models.CharField(max_length=cfg.OrderLine.MAXIMUM_LENGTH_OF_ORDERLINE_STATUS_CHOICES,
                                      choices=ORDERLINE_STATUS_AS_DICT, default=cfg.OrderLine.DEFAULT_STATUS,
                                      help_text="Order Status", null=True, blank=False)
            notes = models.TextField(max_length=cfg.OrderLine.MAXIMUM_NOTES_LENGTH, help_text="Order Line Notes",
            ...
            def create(cls: Union[Self, Callable], linkedOrder: Order, linkedProduct: Product, quantity: int,
                ...
                if (not (isinstance(quantity, int) and (
                        isinstance(persistentCost, (int, float)) or persistentCost is None) and
                         (isinstance(itemCost, (int, float)) or persistentCost is None) and
                         isinstance(forcePrice, (int, float)) or forcePrice is None)):
                    raise TypeError("bad type for quantity, persistentCost, itemCost or forcePrice")
                if ((persistentCost is not None and persistentCost < 0) or (itemCost is not None and itemCost < 0) or
            ...
            def _reserveStock(self: Self, forceAll: bool = False, atLeast: int = None, atMost: int = None,
                              exact: int = None, overrideAttemptUntilStockFound: bool = None,
                              overrideFindMaximumStockAvailable: bool = None,
                              autosavePasson: bool = cfg.DEFAULT_AUTOSAVE_MODE,
                              ) -> int | NoReturn:
                """
                TODO: Implement new multi-timestamp reservation
                Internal function to reserve stock from the linked product. Raises an exception if the operation fails.
            ...
            def doConfirm(self: Self, *_, autosave: bool = cfg.DEFAULT_AUTOSAVE_MODE, autostartTransaction: bool = True,
                ...
                if autostartTransaction:
                    with transaction.atomic():
                        self._confirmOrderLn(confirmedStatus=overrideConfirmedStatus, allowUnpaid=overrideAllowUnapid,
                                             unpaidConfirmedStatus=overrideUnpaidConfirmedStatus,
                                             autosavePasson=autosave)
                        if autosave: self.save()
                else:
                    self._confirmOrderLn(confirmedStatus=overrideConfirmedStatus, allowUnpaid=overrideAllowUnapid,
        ...
        class Transaction(models.Model):
            ...
            id = models.CharField(max_length=45, primary_key=True, null=False, blank=False, unique=True,
                                  help_text="Unique Transaction ID, same across databases")  # TRANSACT-UUID4 (len 45)
            linkedOrderLine = models.ForeignKey(OrderLine, on_delete=models.CASCADE, help_text="OrderLine ID",
                                                default=-1, null=False, blank=False)
            linkedStockConfig = models.ForeignKey(StockConfig, on_delete=models.PROTECT,
                                                  help_text="Stock Configuration ID", default=-1, null=False,
                                                  blank=False)
            quantityReserved = models.PositiveIntegerField(help_text="Quantity Reserved from linkedStockConfig",
                                                           null=False, blank=False, default=0)
            status = models.CharField(max_length=1, choices=Status.choices, default=Status.CONFIRMED, blank=False,
                                      null=False)
            _saveVersion = IntegerVersionField(help_text="Save Version for Concurrency Control")
            ...
            class Meta:
                ...
                permissions = []  # Permissions should carry on from OrderLine model Permissions
                indexes = [
                    models.Index(fields=["id", "linkedOrderLine"], name="CanDB_Transaction_ID_and_OrderLine_Index")]
                verbose_name = "Transaction"
                verbose_name_plural = "Transactions"
                constraints = [
                    models.CheckConstraint(check=models.Q(id__len=45),
        ...
        def __devEnvCreator() -> bool:
            ...
            if not GLOBAL_SETTINGS.DEBUG:
                raise PermissionError("cannot create dev environment executable in debug mode")
            try:
                Profile.create(forceID=1, username="anony", first_name="Anony", last_name="Mous",
                               email="anony@mous.com", password="1234", is_staff=False, is_superuser=False, image=None,
            ...
            try:
                # TODO: migrate availability to timebrackets and stockconfigs
                prod = Product.create(name="Test Product", price=1.00, description="Test Product", image=None,
                                      physicalStock=10, reservedStock=0, notes="Test Product", tags=["test"],
                                      autosave=True,
                                      availability={
                                          1: ((datetime.now(tz=TZ_INFO), datetime.now(tz=TZ_INFO) + timedelta(days=1)),
                                              {"available": True, "physicalStock": 10, "reservedStock": 0,
                                               "infinite": False, "useModelStock": False}),
                                      })
                prod = Product.create(name="Advanced Product 1", price=6.45, description="Test Product", image=None,
                                      physicalStock=13,
                                      reservedStock=0, notes="Test Product", tags=["test"], autosave=True,
                                      availability={
                                          1: (
                                              ...
                                              {"available": False, "physicalStock": 15, "reservedStock": 0,
                                               "infinite": False,
                                               "useModelStock": False}
                                          ),
                                          2: (
                                              (datetime.now(tz=TZ_INFO).now().date(),
                                               datetime.now(tz=TZ_INFO).date() + timedelta(days=3)),
                                              {"available": True, "infinite": True},
                                          ),
                                          3: (
                                              (datetime.now(tz=TZ_INFO).now().weekday(),
                                               datetime.now(tz=TZ_INFO).weekday() + 2),
            ...
            # Create an order
            try:
                od = Order.create(profile=Profile.objects.get(username="anony"), notes="Test Order",
                                  overwriteTime=datetime.now(tz=TZ_INFO), autosave=True)
            except django.db.utils.IntegrityError:
            ...
            try:
                if prodOrOrdFailed:
                    ...
                    OrderLine.create(linkedOrder=od, linkedProduct=prod, quantity=3, notes="Test OrderLine",
                                     autosave=True)
            except django.db.utils.IntegrityError:
